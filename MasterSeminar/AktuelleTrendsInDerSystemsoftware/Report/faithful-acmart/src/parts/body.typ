...
#let apply-body(cfg, body, amsart-lists: false) = context {
  ...
  show figure.where(kind: image): set figure(supplement: if cfg.bibstrip { [Abb.] } else { [Abbildung] })
  ...
  set bibliography(style: "association-for-computing-machinery", title: [Quellenverzeichnis])
